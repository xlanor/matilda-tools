...
function insertDB($target){
	...
	foreach($x->getElementsByTagName('item') as $itemelement) //foreach <item> group,
	{
		...
		$checkq = "SELECT today_link FROM TodayOnline WHERE today_link = :link";
		...
		if (!$row)
			{
				...
				$insertstq = "INSERT INTO TodayOnline VALUES(NULL,:title,:link,:pubtime,:cat)";
...
function delVid(){
	include('connection.php');
	$deleteq = "DELETE FROM TodayOnline WHERE today_cat = 'Videos'";
