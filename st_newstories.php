...
function insertdb($url, $category){
	...
	{
		$checkq = "SELECT url_link FROM combinedarticle WHERE url_link = :link AND url_site = 1";
		...
		if (!$row)
		{
			...
			$insertstq = "INSERT INTO combinedarticle VALUES(NULL,:title,:link,:pubtime,:cat,1)";
